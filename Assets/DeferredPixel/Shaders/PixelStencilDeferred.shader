Shader "Hidden/Deferred Pixel/StencilDeferred"
{
    Properties {
    ...
    half4 DeferredShading(Varyings input) : SV_Target
    {
        ...
        screen_uv = SAMPLE_TEXTURE2D_X_LOD(_PixelTexture, my_point_clamp_sampler, screen_uv, 0);
        input.positionCS = float4(screen_uv * _ScreenSize, input.positionCS.z, input.positionCS.w);
        ...
        #if defined(_LIT)
            clip((materialFlags & kMaterialFlagPixel) == 0 ? 1 : -1);
